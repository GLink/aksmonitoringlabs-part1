...
```powershell
# Create and apply a CPU stress deployment
@"
apiVersion: apps/v1
kind: Deployment
metadata:
...
Write-Host $LOG_ANALYTICS_WORKSPACE_ID
```
...
kubectl get pods -n demo-apps -o json | jq -r '.items[] | "\(.status.containerStatuses[0].restartCount // 0) \(.metadata.name) \(.status.phase)"' | Sort-Object { [int](($_ -split ' ')[0]) } -Descending
```
...
```powershell
# Create and apply an OOMKill scenario by modifying our memory stress app
@"
apiVersion: apps/v1
kind: Deployment
metadata:
...
kubectl describe pod -n demo-apps -l app=memory-oom | Select-String -Pattern "Last State" -Context 0,5
...
```powershell
# Create and apply the broken crashloop-demo deployment
@"
apiVersion: apps/v1
kind: Deployment
metadata:
...
kubectl describe pod -n demo-apps -l app=crashloop-demo | Select-String -Pattern "Events" -Context 0,10
...
- Verify ama-logs pods are running: `kubectl get pods -n kube-system | Select-String "ama-logs"`
