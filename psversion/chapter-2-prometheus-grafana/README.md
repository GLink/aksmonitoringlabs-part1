...
$AZURE_MONITOR_WORKSPACE = "amw-${STUDENT_INITIALS}-prometheus"
...
$AZURE_MONITOR_WORKSPACE_ID = $(az resource show `
  --resource-group $RESOURCE_GROUP `
  --namespace microsoft.monitor `
  --resource-type accounts `
  --name $AZURE_MONITOR_WORKSPACE `
...
Write-Host "Azure Monitor Workspace ID: $AZURE_MONITOR_WORKSPACE_ID"
```
...
$GRAFANA_NAME = "grafana-${STUDENT_INITIALS}"
...
Write-Host "Grafana instance creation started..."
```
...
$PROMETHEUS_QUERY_ENDPOINT = $(az monitor account show `
  --name $AZURE_MONITOR_WORKSPACE `
  --resource-group $RESOURCE_GROUP `
  --query metrics.prometheusQueryEndpoint `
  --output tsv)

Write-Host "Prometheus Query Endpoint: $PROMETHEUS_QUERY_ENDPOINT"

# Delete existing data source if it exists (to avoid conflicts)
az grafana data-source delete `
  --name $GRAFANA_NAME `
  --resource-group $RESOURCE_GROUP `
  --data-source "Azure Managed Prometheus" 2>$null
...
Write-Host "Grafana URL: $GRAFANA_ENDPOINT"
Write-Host ""
Write-Host "Copy and paste the URL above into your browser"
```
...
>   --output tsv)
> 
> Write-Host "Using endpoint: $PROMETHEUS_QUERY_ENDPOINT"
> 
